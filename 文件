...
# 配置
# -------------------------
STATION_FILE = Path("station_data")   # 不写后缀，自动尝试 .csv/.xls/.xlsx
COUNTS_DIR   = Path("counts-data")    # 读取该文件夹下的所有 csv/xls/xlsx
OUTPUT_DIR   = Path("outputs")

RADIUS_M = 500  # 覆盖半径（米）
AM_HOURS = set(range(7, 10))    # 07-09
PM_HOURS = set(range(16, 20))   # 16-19
...
def main():
    ...
    # 仅保留 counts 中出现在 station 列表里的起点（避免脏 ID）
